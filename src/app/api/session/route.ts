...
import { z } from 'zod';
import { SESSION_COOKIE, SESSION_TTL_MS, passwordMatches, signSession } from '@/lib/session';
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const token = signSession(Date.now() + SESSION_TTL_MS, process.env.SESSION_SECRET);
